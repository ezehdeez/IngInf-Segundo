...
void OpSelector(std::ifstream& input_file, std::ofstream& output_file, int opcode) {
  ...
  bool op6 = false;
  while(input_file >> chain_string >> alphabet_string && !op6) {
    ...
    switch(opcode) {
      ...
      case 5:
        ...
        break;
      case 6:
        std::cout << "Write your subsequence: " << std::endl;
        std::cin >> str;
        std::cout << (target_chain.SearchSubsequence(str) ? "String found" : "String not found") << std::endl;
        op6 = true;
        break;
      default:
