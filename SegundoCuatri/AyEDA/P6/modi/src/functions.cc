...
void PrintUsage(const std::string& program_name) {
  std::cout << "\nUso: " << program_name << " -ab <tipo_arbol> [opciones]\n"
            ...
            << "  " << program_name << " -ab abb -init manual\n\n";
}
...
AB<NIF>* BuildTree(const ProgramConfig& config) {
  ...
  throw std::runtime_error("[ERROR]: Tipo de árbol no soportado (" + config.tree_type + "). Usa 'abb' o 'abe'.");
}

void InitializeTree(AB<NIF>* arbol, const ProgramConfig& config) {
  if(config.init_mode == "manual") {
    std::cout << "--- INICIALIZACIÓN MANUAL ---\n";
    for(size_t i = 0; i < config.element_amount; i++) {
      long valor;
      std::cout << "Introduce el valor " << i + 1 << "/" << config.element_amount << ": ";
      std::cin >> valor;
  ...
  while (opcion != 0) {
    ...
    switch(opcion) {
      ...
      case 4:
        ...
        std::cin >> valor;
        if(arbol->borrar(NIF(valor))) {
          std::cout << "[RESULTADO]: La clave " << valor << " se ha borrado con éxito.\n";
        } else {
          std::cout << "[RESULTADO]: La clave " << valor << " no se ha podido borrar. Compruebe que existe usando la operación buscar (2)." << std::endl;
        }
        std::cout << "\nESTADO ACTUAL DEL ÁRBOL (Niveles):\n";
        std::cout << *arbol;
        break;
